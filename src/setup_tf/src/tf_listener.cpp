...
const int frameNum = 2; //number of cameras in system
string frameName[] = {"camera_1","camera_2"}; //namespacing of cameras

const int tagNum = 5; //number of tags in system
const int robotId = 4;// index of robot tag (not used to create map)
string tagName[]={"tag0","tag1","tag4","tag5","tag6"}; //name of all the tags in the system
...
int obsNum = 2;
int obsTags[] = {0,1}; //index of obstical tags
    ...
    try{
		...
		for (int i=0; i<tagNum; i++)
		{
			if(i==robotId)
			{
				...
				//broadcast
				out_pose.setOrigin( tf::Vector3(world[i].x,world[i].y,world[i].roll));
				rQ.setRPY(0,0,0);
				out_pose.setRotation(rQ);
				br.sendTransform(
				tf::StampedTransform(out_pose, ros::Time::now(), "world", "robot"));
				//cout<<"Robot X:"<<world[i].x<<" Y:"<<world[i].y<<"\n";
				//std::cout<<"angle: "<<world[i].roll<<"\n";
				//display location
			...
			if(i==obsTags[0])
			{
				...
				out_pose.setRotation(rQ);
				br.sendTransform(
				tf::StampedTransform(out_pose, ros::Time::now(), "world", ("obs_1")));
				//cout<<"obs_1 X:"<<world[i].x<<" Y:"<<world[i].y<<"\n";
			}
			if(i==obsTags[1])
			{
				tf::Transform out_pose;
				...
				out_pose.setRotation(rQ);
				br.sendTransform(
				tf::StampedTransform(out_pose, ros::Time::now(), "world", ("obs_2")));
				//cout<<"obs_2 X:"<<world[i].x<<" Y:"<<world[i].y<<"\n";
			}
		}
